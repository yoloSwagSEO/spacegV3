...
class Fleets
{
	...
	public static function target_distance ( $orig_galaxy , $dest_galaxy , $orig_system , $dest_system , $orig_planet , $dest_planet )
	{
		...
		$distance = sqrt ( pow ($dest['x']-$dept['x'],2)+pow ($dest['y']-$dept['y'],2))/10;
	...
	/**
	 * method fleet_capacity
	 * param1 $ship
	 * param2 $user
	 * return fleet capacity
	 */
	public static function fleet_capacity($ship, $user){
		global $pricelist;
		return $pricelist[$ship]['capacity'];
	}
	/**
	...
	public static function fleet_max_speed ( $fleet_array , $fleet , $user )
	{
		...
		$fleet_array = (is_array($fleet_array))?$fleet_array:array();
		if ( $fleet != 0 )
		{
			$fleet_array[$fleet] =  1;
		}
		...
		{
			if ( in_array($ship,$reslist['propVx'][0])  )
			{
				if ( $user['impulse_motor_tech'] >= 5 )
				{
			...
			if ( in_array($ship,$reslist['propVx'][1]) )
			{
				$speed_all[$ship] 		= $pricelist[$ship]['speed'] + ( ( $pricelist[$ship]['speed'] * $user['combustion_tech'] ) * 0.1 );
			}
				
	
			if ( in_array($ship,$reslist['propVx'][2]) )
			{
				$speed_all[$ship] 		= $pricelist[$ship]['speed'] + ( ( $pricelist[$ship]['speed'] * $user['impulse_motor_tech'] ) * 0.2 );
			}
	
			if ( in_array($ship,$reslist['propVx'][3]) )
			{
			...
			if ( in_array($ship,$reslist['propVx'][4]) )
			{
	...
	public static function fleet_consumption ( $fleet_array , $speed_factor , $mission_duration , $mission_distance , $fleet_max_speed , $user )
	{
		...
		$basic_consumption 	= 0;
		$b_conso = 0;
		//echo 'distance'. $mission_distance;
		foreach ( $fleet_array as $ship => $count )
		{
