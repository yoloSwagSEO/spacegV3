...
class GalaxyRows
{
	...
	public function GalaxyRowActions($GalaxyInfo, $Galaxy, $System, $Planet, $CurrentGalaxy, $CurrentSystem, $CurrentMIP)
	{
		...
		if ($GalaxyInfo['id'] != $user['id'] && $GalaxyInfo['id'] != 0)
		{
			if ($CurrentMIP <> 0)
			{
				if ($GalaxyInfo['id'] != $user['id'] && $GalaxyInfo['id'] != 0)
			...
			{
				if (($user["settings_esp"] == "1" && $GalaxyInfo['id']) && $GalaxyInfo['id'] != 0)
				{
					$links .= "<a href=# onclick=\"javascript:doit(6, ".$Galaxy.", ".$System.", ".$Planet.", 1, ".$user["spio_anz"].");\" >";
					$links .= "<img src=". DPATH ."img/spion.png width=\"35px\" height=\"35px\" title=\"".$lang['gl_spy']."\" border=0></a>";
					$links .= "&nbsp;";
				}
				if ($user["settings_wri"] == "1" && $GalaxyInfo['id'] && $GalaxyInfo['id'] != 0)
				{
					$links .= "<a href=game.php?page=messages&mode=write&id=".$GalaxyInfo["id"].">";
					$links .= "<img src=". DPATH ."img/message.png width=\"35px\" height=\"35px\" title=\"".$lang['write_message']."\" border=0></a>";
					$links .= "&nbsp;";
				}
				if ($user["settings_bud"] == "1" && $GalaxyInfo['id'] && $GalaxyInfo['id'] != 0)
				{
					$links .= "<a href=game.php?page=buddy&mode=2&u=".$GalaxyInfo['id']." >";
					$links .= "<img src=". DPATH ."img/friend.png width=\"35px\" height=\"35px\" title=\"".$lang['gl_buddy_request']."\" border=0></a>";
					$links .= "&nbsp;";
				}

				if ($user["settings_mis"] == "1" && $MissileBtn == TRUE && $GalaxyInfo['id'] && $GalaxyInfo['id'] != 0)
				{
                ...
                if($GalaxyInfo['id'] == ''){
					$links .= "<a href=\"#\" onclick=\"javascript:doit(16, ".$Galaxy.", ".$System.", ".$Planet.", 1, ".$user["spio_anz"].");\" class=\"doit\" data-param=\"16_".$Galaxy."_".$System."_".$Planet."_1_".$user["spio_anz"]."\">";
                    $links .= "<img src=". DPATH ."img/explore.png width=\"35px\" height=\"35px\" title=\"Cibler\" border=0></a>";
                    $links .= "&nbsp;"; 
                }
                                $links .= "<a href=game.php?page=fleet&galaxy=".$Galaxy."&system=".$System."&planet=".$Planet." >";
				$links .= "<img src=". DPATH ."img/target.png width=\"35px\" height=\"35px\" title=\"Cibler\" border=0></a>";
				$links .= "&nbsp;";
		return $links;
	}
	...
	public function GalaxyRowPlanet($GalaxyInfo, $Galaxy, $System, $Planet, $PlanetType, $HavePhalanx, $CurrentGalaxy, $CurrentSystem)
	{
		...
		if ($GalaxyInfo && $GalaxyInfo["destruyed"] == 0)
		{
			...
			if ($GalaxyInfo['id']  && $GalaxyInfo['id'] != 0)
			...
			$parse['name_js']	= str_replace('\'','',$GalaxyInfo["name"]);
			$parse['galaxy']	= $Galaxy;
			$parse['system']	= $System;
			$parse['planet']	= $Planet;
	...
	public function GalaxyRowPlanetName($GalaxyInfo, $Galaxy, $System, $Planet, $PlanetType, $HavePhalanx, $CurrentGalaxy, $CurrentSystem)
	{
		...
		if ($GalaxyInfo['last_update'] > (time()-59 * 60) && $GalaxyInfo['id'] != $user['id'] && $GalaxyInfo['id'] != 0)
		{
