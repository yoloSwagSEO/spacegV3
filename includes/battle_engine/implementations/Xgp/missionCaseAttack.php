...
if ($FleetRow['fleet_mess'] == 0 && $FleetRow['fleet_start_time'] <= time())
{
    ...
    $targetPlanet = doquery("SELECT * FROM {{table}} WHERE `galaxy` = " . $FleetRow['fleet_end_galaxy'] . " AND `system` = " . $FleetRow['fleet_end_system'] . " AND `planet` = " . $FleetRow['fleet_end_planet'] . ";", 'planets', true);
...
elseif ($FleetRow['fleet_end_time'] <= time())
{
 ...
 //   SendSimpleMessage($FleetRow['fleet_owner'], '', $FleetRow['fleet_end_time'], 3, $lang['sys_mess_tower'], $lang['sys_mess_fleetback'], $Message);
    $this->RestoreFleetToPlanet($FleetRow,true,true);
...
function getPlayerGroup($fleetRow)
{
    ...
    $fighter = unserialize($fleetRow['fleet_fighter']);
    if(is_array($fighter)){
        foreach ($fighter AS $key => $value){
            if ($key != 0 && $value != 0)
        {
            $fleet->add(getShipType($key, $value));
        }
        }
    }
    ...
    $player->setName($player_info['username']);
...
function sendMessage($FleetRow, $report, $lang, $resource)
{
    ...
    foreach ($idAtts as $id)
    {
        ...
        SendSimpleMessage($FleetRow['fleet_owner'], 0, $FleetRow['fleet_start_time'], 3, $lang['sys_mess_tower'], "Rapport de combat", $raport);
    ...
    foreach ($idDefs as $id)
    {
        ...
        $raport = "<a href=\"#\" style=\"color:" . $style . ";\" OnClick=\'f(\"CombatReport.php?raport=" . $rid . "\", \"\");\' >" . $lang['sys_mess_attack_report'] . " [" . $FleetRow['fleet_end_galaxy'] . ":" . $FleetRow['fleet_end_system'] . ":" . $FleetRow['fleet_end_planet'] . "]</a>";
        SendSimpleMessage($id, 0, $FleetRow['fleet_start_time'], 3, $lang['sys_mess_tower'], "Rapport de combat", $raport);
    }
