	...
	function CheckPlanetBuildingQueue ( &$CurrentPlanet, &$CurrentUser )
	{
		...
		if ($CurrentPlanet['b_building_id'] != 0)
		{
			...
			if ($BuildEndTime <= time())
			{
				...
				elseif ($CurrentPlanet['planet_type'] == 1)
				{
					if ($ForDestroy == FALSE)
					{
						...
						if(in_array($Element,$reslist['build'] )){
							do_action('pre-building-end',array('element'=>$Element,'lvl'=>$CurrentPlanet[$resource[$Element]]));
						}
					}
					else
					{
						$current -= 1;
						$CurrentPlanet[$resource[$Element]]--;
						do_action('pre-building-destroy-end',array('element'=>$Element,'lvl'=>$CurrentPlanet[$resource[$Element]]));
					}
				}
				...
				do_action('post-building-end',array('element'=>$Element,'lvl'=>$CurrentPlanet[$resource[$Element]],'destruc'=>$ForDestroy));
