...
class ShowGalaxyPage extends GalaxyRows
{
	...
	public function __construct($CurrentUser, $CurrentPlanet)
	{
		...
		elseif ($mode == 1)
		{
			...
			if ($postedGalaxy > $maxGalaxy[0])
				$postedGalaxy = $maxGalaxy[0];

			if ($postedSystem > $maxSystem[0])
				$postedSystem = $maxSystem[0];
			...
			elseif ($galaxyRight)
			{
				...
				{
					$postedGalaxy  	= $maxGalaxy[0];
					$galaxyRight 	= $maxGalaxy[0];
					$galaxy       	= $maxGalaxy[0];
				}
				elseif ($postedGalaxy == $maxGalaxy[0])
				{
			...
			elseif ($systemRight)
			{
				...
				elseif ($postedSystem == $maxSystem[0])
				{
					$postedSystem  	= $maxSystem[0];
					$system       	= $maxSystem[0];
				}
		...
		$GalaxyInfo = doquery( "SELECT {{table}}galaxy.metal, {{table}}galaxy.crystal, {{table}}galaxy.id_luna, {{table}}galaxy.id_planet, {{table}}planets.galaxy, {{table}}planets.system, {{table}}planets.planet, {{table}}planets.destruyed, {{table}}planets.name, {{table}}planets.image, {{table}}planets.last_update,{{table}}planets.id_owner,{{table}}users.id, {{table}}users.ally_id, {{table}}users.bana, {{table}}users.urlaubs_modus, {{table}}users.onlinetime, {{table}}users.username,{{table}}statpoints.stat_type, {{table}}statpoints.stat_code, {{table}}statpoints.total_rank, {{table}}statpoints.total_points, {{table}}moons.diameter, {{table}}moons.temp_min, {{table}}moons.destruyed AS destruyed_moon, {{table}}moons.name AS name_moon, {{table}}alliance.ally_name, {{table}}alliance.ally_tag, {{table}}alliance.ally_web, {{table}}alliance.ally_members,{{table}}buddy.owner AS friends_owner,{{table}}buddy.sender AS friends_sender
			FROM {{table}}alliance RIGHT JOIN ({{table}}planets AS {{table}}moons RIGHT JOIN ({{table}}statpoints RIGHT JOIN ((({{table}}planets LEFT JOIN {{table}}users ON {{table}}planets.id_owner = {{table}}users.id ) LEFT JOIN {{table}}galaxy ON {{table}}planets.id = {{table}}galaxy.id_planet ) LEFT JOIN {{table}}buddy ON ({{table}}buddy.owner = {{table}}planets.id_owner OR {{table}}buddy.sender = {{table}}planets.id_owner ))  ON {{table}}statpoints.id_owner={{table}}users.id AND {{table}}statpoints.stat_code=1 AND {{table}}statpoints.stat_type=1 ) ON {{table}}moons.id = {{table}}galaxy.id_luna) ON {{table}}alliance.id = {{table}}users.ally_id
			WHERE ({{table}}galaxy.galaxy='".$galaxy."' AND {{table}}galaxy.system='".$system."')
			GROUP BY `id_planet`
			ORDER BY {{table}}planets.planet; " , '' );
		
		//echo "SELECT {{table}}galaxy.metal, {{table}}galaxy.crystal, {{table}}galaxy.id_luna, {{table}}galaxy.id_planet, {{table}}planets.galaxy, {{table}}planets.system, {{table}}planets.planet, {{table}}planets.destruyed, {{table}}planets.name, {{table}}planets.image, {{table}}planets.last_update,{{table}}planets.id_owner,{{table}}users.id, {{table}}users.ally_id, {{table}}users.bana, {{table}}users.urlaubs_modus, {{table}}users.onlinetime, {{table}}users.username,{{table}}statpoints.stat_type, {{table}}statpoints.stat_code, {{table}}statpoints.total_rank, {{table}}statpoints.total_points, {{table}}moons.diameter, {{table}}moons.temp_min, {{table}}moons.destruyed AS destruyed_moon, {{table}}moons.name AS name_moon, {{table}}alliance.ally_name, {{table}}alliance.ally_tag, {{table}}alliance.ally_web, {{table}}alliance.ally_members,{{table}}buddy.owner AS friends_owner,{{table}}buddy.sender AS friends_sender
		...
		while ( $GalaxyInfo = mysqli_fetch_array ( $GalaxyQuery ) )
		{
			for ($Planet = $start; $Planet < $numPlanet; $Planet++)
			{
				...
				if ( $GalaxyInfo['galaxy'] == $Galaxy && $GalaxyInfo['system'] == $System && $GalaxyInfo['planet'] == $Planet )
				{
					...
					{
						if ($GalaxyInfo['destruyed'] != 0 && $GalaxyInfo['id_owner'] != '' )
						{
                    ...
                    if ( $GalaxyInfo['destruyed'] == 0 )
                    {
						...
						if($GalaxyInfo['id_owner'] != 0){
							$parse['moon'] = file_exists ('styles/images/emblem/mini/'.$GalaxyInfo['id_owner'].'_32.png')?'<img src="styles/images/emblem/mini/'.$GalaxyInfo['id_owner'].'_32.png" />':'<img src="styles/images/emblem/mini/none_32.png" />';
						}else{
							$parse['moon'] = "";
						}
