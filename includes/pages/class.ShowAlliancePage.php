		...
		if ( $CurrentUser['ally_id'] != 0 && $CurrentUser['ally_request'] == 0 )
		{
			...
			if ( $mode == 'admin' && $edit == 'name' )
			{
				...
				if ( isset($_POST['nom']) && !empty($_POST['nom'] ) )
				{
					...
					$ally['ally_name']	= mysql_escape_value ( strip_tags ( $_POST['nom'] ) );
			...
			if ( $mode == 'admin' && $edit == 'tag')
			{
				...
				if ( isset ( $_POST['tag'] ) && ( strlen ( $_POST['tag'] ) < 3 OR strlen ( $_POST['tag'] ) > 8 ) )
				{
				...
				{
					doquery ( "UPDATE {{table}} SET `ally_tag` = '". mysql_escape_value ( strip_tags ( $_POST['tag'] ) ) . "' WHERE `id` = '" . $CurrentUser['ally_id'] . "';" , 'alliance' );
				}
