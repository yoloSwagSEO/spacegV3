...
{ 
	function __construct ( $CurrentUser , $CurrentPlanet )
	{
		...
		$lunarow = doquery("SELECT * FROM {{table}} WHERE 
									`id_owner` = '" . intval($CurrentPlanet['id_owner']) . "' 
								AND `galaxy` = '" . intval($CurrentPlanet['galaxy']) . "' 
								AND `system` = '" . intval($CurrentPlanet['system']) . "' 
								AND  `planet` = '" . intval($CurrentPlanet['planet']) . "'
								AND `planet_type`='3'",
			             'planets',TRUE);
		...
		switch((isset($_GET['mode'])?$_GET['mode']:NULL))
		{
			...
			default:
				...
				//echo $parse['planet_field_max'];
                                //$parse['info_colo'] =  $lang['ov_info'];
				
				 $parse['case_pourcentage'] = floor($CurrentPlanet['field_current'] / CalculateMaxPlanetFields($CurrentPlanet) * 100) . '%';
				// Barre de remplissage
				$parse['case_barre'] = floor($CurrentPlanet['field_current'] / CalculateMaxPlanetFields($CurrentPlanet) * 100) * 6.5;
			if ($parse['case_barre'] > (100 * 6.5)) {
                $parse['case_barre'] = 650;
                $parse['case_barre_barcolor'] = '#C00000';
            } elseif ($parse['case_barre'] > (80 * 6.5)) {
				...
				while($data = mysqli_fetch_array($message)){
					...
					//print_r($data);
					//echo '</pre>';
					if($messageColor[$data['message_type']]){
						$background = 'background:'.$messageColor[$data['message_type']].';';
					}else{
						$background = '';
					}
                                        
                                            $data['message_hef'] = 'game.php?page=messageview&id='.$data['message_id'].'';
