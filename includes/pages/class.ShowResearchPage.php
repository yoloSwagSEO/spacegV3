	...
	private function HandleTechnologieBuild ( &$CurrentPlanet, &$CurrentUser )
	{
		...
		if ($CurrentUser['b_tech_planet'] != 0)
		{
			...
			if ($ThePlanet['b_tech']    <= time() && $ThePlanet['b_tech_id'] != 0)
			{
				...
				do_action('pre-research-end',array('element'=>$Element,'lvl'=>$CurrentUser[$resource[$ThePlanet['b_tech_id']]]));
		...
		foreach($lang['tech'] as $Tech => $TechName)
		{
			if ($Tech > 105 && $Tech <= 199)
			{
				...
				if ( IsTechnologieAccessible($CurrentUser, $CurrentPlanet, $Tech))
				{
					...
					if($Tech == 106)
					{
						$RowParse['tech_level']  = ($building_level == 0 ) ? "" : "(".$building_level .")" ;
					...
					if (!$InResearch)
					{
						...
						if ($CanBeDone)
						{
							...
							else
							{
								...
								if ($LevelToDo == 1){
									$TechnoLink .= "<img src=\"styles/skins/xgproyect/images/Up2.png\" />";
								}else{
									$TechnoLink .= "<img src=\"styles/skins/xgproyect/images/Up2.png\" />";
									$nobuild = '';
								}
