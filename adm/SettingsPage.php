...
function DisplayGameSettingsPage ( $CurrentUser )
{
	...
	if ( isset($_POST['opt_save']) && $_POST['opt_save'] == "1" )
	{
		...
		//Si on change la langue, il faut effectuer quelques modifications concernant le nom des planètes principales.
		if(DEFAULT_LANG != $_POST['language'])
		{
			//On récupère le nom actuel
			$old_planetname = $lang['main_planet'];
			
			//On change la langue
			includeLang ( 'COMMON', $_POST['language'] );
			
			//On récupère le nouveau nom
			$new_planetname = $lang['main_planet'];
			
			//On modifie la valeur par défaut.
			doquery ( "ALTER TABLE {{table}} CHANGE `name` `name` varchar(255) character set latin1 default '$new_planetname'" , 'planets' );
			
			//On termine en modifiant les noms de planète.
			doquery ( "UPDATE {{table}} SET `name` = '$new_planetname' WHERE `name` = '$old_planetname'" , 'planets' );
