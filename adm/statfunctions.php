...
/*	global $resource, $pricelist, $reslist;
	...
	if($ChooseToDelete)
	{
		...
		while($delete = mysqli_fetch_array($ChooseToDelete))
		{
	...
	for ($Query=1;$Query<=$LastQuery;$Query++)
	{
		...
		while ($CurPlanet = mysqli_fetch_array($parcial_data))
		{
		...
		while ($CurStats = mysqli_fetch_array($old_stats))
		{
		...
		if($game_stat_flying == 1)
		{
			...
			while ($CurFleets = mysqli_fetch_array($flying_fleets))
			{
		...
		while ($CurUser = mysqli_fetch_array($total_data))
		{
	...
	$total_ally		=0;
	while ($CurAlly = mysqli_fetch_array($ally_check))
	{
		++$total_ally;
	...
	if ($total_ally > 0)//We only update allys if at least 1 ally exist...
	{
		...
		for ($Query=1;$Query<=$LastQuery;$Query++)
		{
			...
			while ($CurAlly = mysqli_fetch_array($ally_data))
			{
			...
			$insert_ally_query	=	"INSERT INTO {{table}}
									...
									`fleet_old_rank`, `fleet_points`, `fleet_count`,
									`total_old_rank`, `total_points`, `total_count`, `stat_date`) VALUES ";
			while ($CurAlly = mysqli_fetch_array($ally_points))
			{
...
function MakeNewRanks($stat_type)
{
	...
	while ($CurUser = mysqli_fetch_array($RankQry) )
	{
